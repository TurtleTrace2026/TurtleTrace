...
export function MonthView({ events, selectedDate, onSelectDate, onAddEvent }: MonthViewProps) {
  ...
  const monthEventCount = useMemo(() => {
    ...
    return events.filter(e => e.date >= startDate && e.date <= endDate).length;
  }, [events, currentMonth]);
        ...
        {calendarDays.map((date, index) => {
          ...
          return (
            <button
              ...
              className={cn(
                "aspect-square rounded-lg flex flex-col items-center justify-start text-sm relative transition-all pt-1",
                isSelected
